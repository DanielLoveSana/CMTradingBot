...
module.exports = class Client {
   ...
   * @prop {string} [proxy] Proxy URL (for example: 'socks5://127.0.0.1:10808')
  ...
  constructor(clientOptions = {}) {
    ...
    const agent = createProxyAgent(clientOptions.proxy);
    ...
    this.#ws.on('close', (code, reason) => {
      this.#logged = false;
      if (code !== 1000) {
        const text = reason ? reason.toString() : 'no reason';
        this.#handleError('WebSocket closed:', `code=${code}`, text);
      }
      this.#handleEvent('disconnected');
    });

    this.#ws.on('error', (err) => {
      this.#handleError('WebSocket error:', formatWSError(err));
    });
